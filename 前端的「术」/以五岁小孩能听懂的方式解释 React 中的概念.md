...
通过建立联系（新旧之间），可以让学习的过程更加轻松、高效。

但每个人知识背景不同，建立联系的方式也不同。前置知识掌握的越多，联系的建立越多也越容易，这也是[之前文章](https://zhuanlan.zhihu.com/p/466213908)强调基础的原因：基础就像这些链接的中心。

5 岁小孩的认识十分有限，一个概念，如果解释到他们都能听懂，实在不易 —— 要对概念特性有深刻把握，以至于外延到 5 岁小孩的认知范围。

就在前几周，试图给我家娃（也快 5 岁了）讲「惯性」：
大山很难推动，海浪很难停下，不动的东西难以移动，动的东西却又难停下，这就是惯性。看着小脸充满迷惑，赶紧补充到，像你在外面玩，一直玩不愿意回来，回家了，又看电视一直不愿意出去。
...
想象你给两个好盆友（Alice & Bob）视频，如果只有一个手机，只能一个一个来：

![](2022-05-19-20-32-51.png)

现在爸爸妈妈的手机都给你了，两部手机，你可以一个手机拨给 Alice，一个拨给 Bob：

![](2022-05-19-20-35-15.png)

两部手机的视频方式，叫并行。

虽然只有一个嘴巴，但是可以一会和 Alice 说，一会和 Bob 说，看似和只有一个手机没区别，但如果是下面的情况：

你特想和 Alice 一块玩泥巴，电话约时间，但 Alice 要和妈妈商量下，让你不要挂电话。
等 Alice 回来的时间很无聊，这时如果有另一部手机，猜猜你可以做什么：

电话好朋友 Bob，聊聊昨晚做的梦，打发时间。

当 Alice 回来，你可以立即接上，又不会因为等待无所事事。

和 Alice 打电话你觉得重要，类似重要的事，在 React 中用 setState；和 Bob 闲聊不重要，类似的用 startTransition，表示有重要事时，可以随时打断。
...
过了一会又发现没有面包，又跑超市买了面包。
接着发现饼干没了，又跑一趟...

终于，做好了早餐，可以开吃了。

但是有没有发现，你可以更短的时间做好早餐，而且不用跑那么多路？

把所有要买的东西提前想好，然后跑到超市一块买回来，然后做早餐。就像妈妈一直给你说的，凡事要有规划，永远不要让重要的事是第一次。
...
其实你不能，除非妈妈给了你菜谱，上面画上每个需要的东西。

在 React 中也一样，怎么知道 setState 结束，可以 render 了呢？
...
如果总是担心遗漏什么，可能想到了中午都没出门买东西，如何能做好早餐呢？

相反，不假思索，刚想一会就跑出去，遗漏了，岂不要再跑一趟？

所以，你只能给自己一个时间点，比如 8 点做好饭，那么无论如何要 7 点前出去买东西。

同样的，如果 setState 没在 event handlers 里，其实 React 也不知道啥时候结束，它只能选一个时间点，既不能立即（每次 setState 就 render），也不能太晚：React 选择 microtask 结束后。

随着你做早餐的次数变多，越不容易遗漏，预估的时间点也越准。React 这里就不如你了，它并不会因为 render 了很多次，变的聪明。
...
在家吃饭，妈妈要准备菜、葱姜蒜，做好了才能开吃。客户端渲染也是这样，准备好渲染页面需要的材料 —— html/css/js，浏览器一顿烹饪，好看的页面就出来了。

在外面吃饭，就省事多了，等者把饭端上就可以开吃了。服务端渲染也是这样，浏览器不用费劲烹饪，需要做的只是开吃 —— 给页面绑定用于交互的事件。

### PROMISE TICK

上次去买奶茶，服务员是不是给了一个小圆盘，当我们的奶茶做好后，小圆盘会响，这时候就可以到柜台取奶茶喝了。

类似的，PROMISE TICK 就是小圆盘，告诉我们「奶茶」做好了。

如果仔细观察，会发现，服务员并不是在奶茶做好后，小圆盘立即就响了，而是等服务员结束了手头正在忙的东西（比如新顾客的买单），才按响了小圆盘。
...
DEBOUNCING 和 THROTTLING 都是「插话办法」。

想象妈妈给你讲故事，但是妈妈希望讲的时候你能和她一块互动，但是你选择什么时候和妈妈互动呢？
...
如果你只是插一句很短的话，比如「太棒了」，没什么问题，但是如果你插了很长的话，十分影响妈妈把故事讲完。
...
这个插话的办法（叫 Debounced）很好，但前提是，妈妈会故意停顿，暗示你接话。如果妈妈不停顿呢？
...
为了避免永远接不上话，你可以这样，拿个小闹钟，一看到妈妈讲了一分钟，立马打断妈妈：

![](2022-05-23-16-46-10.png)

这个插话办法叫 Throttled，在妈妈滔滔不绝的时候十分有用。但是，如果妈妈已经停下，暗示你说话，你却还在掐着小闹钟计时，岂不很尴尬：
...
dan 提出一个视角来理解 server component：
想象我们的组件树，从一开始只能在 client 运行，变成了，可以自由（当然也不是完全自由，能在服务端运行的组件有特殊要求）指定哪些运行在服务端，哪些继续运行在 client：
...
在函数内声明的变量，随着函数执行完而消失（除闭包外），下次调用又是新的。
如果这些变量保存了有用信息，不想随着函数消失怎么办？**把变量提到函数外面**。

函数外的变量，就是 state，它们保持了组件内要持久化的信息（下次渲染要使用）。
